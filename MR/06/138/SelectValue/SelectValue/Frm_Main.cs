...
namespace SelectValue
{
    public partial class Frm_Main : Form
    {
        ...
        {
            int Age;//定义变量
            if (int.TryParse(txt_Age.Text, out Age))//为变量赋值
            {
                dgv_Message.DataSource = GetMessageWithAge(Age);//设置数据源
            }
            else
            {
                MessageBox.Show("请输入正确数值", "提示！");//弹出消息对话框
            }
        }
        ...
        private DataTable GetMessageWithAge(int Age)
        {
